				...
				<>
					<ContentWrap>
						<Container>
							<Content>
								<H1
									font={isMobile ? '2.1875rem' : '3.8125rem'}
									textCenter={isMobile ? 'center' : 'left'}
									top='3rem'
									bottom={isMobile ? '1rem' : '2rem'}>
									Explore the world with Geotagger!
								</H1>
								<P
									max={isMobile ? '510px' : '370px'}
									textCenter={isMobile ? 'center' : 'left'}>
									Geotagger is webiste that allowes you to post picture and tag it on the
									map. Other user than try to locate it via Google Maps.{' '}
								</P>
								<LinkStyled
									to='signup'
									center={isMobile ? 'center' : 'left'}
